...
public class ItemDeConsumoController {
	...
	public String gravarItem(@Valid GravarItem requisicao, BindingResult resultadoValidacao) {
		...
		if (resultadoValidacao.hasErrors()) {
			...
			System.out.println(String.valueOf(LocalDateTime.now()).concat("  ")
					.concat(String.valueOf(resultadoValidacao.getFieldError())));
			return "item/form-novo";
		}

//		ItemDeConsumo novoItem = requisicao.toItemDeConsumo(dispensaRepo);
	...
	public String gravarLista(@Valid GravarLista requisicao, BindingResult resultadoValidacao) {
		...
		/*
		 * Valida o processo de verificação dos atributos preenchidos e redireciona para
		 * o formulário para correção.
		 */
		if (resultadoValidacao.hasErrors()) {
			System.out.println(String.valueOf(LocalDateTime.now()).concat("  ")
					.concat(String.valueOf(resultadoValidacao.getFieldError())));
